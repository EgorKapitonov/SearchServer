...
public:
    ...
    //FindTopDocuments(raw_query, document_predicate)
    ...
    //Методы FindTopDocuments с приемом документов execution::seq и execution::par (const ExecutionPolicy& policy)
...
private:
    ...
    std::map<int, std::map<std::string_view, double>> document_to_word_freqs_; //Словарь частот слов для GetWordFrequencies
    ...
    //Методы FindAllDocuments с приемом документов execution::seq и execution::par
    template <typename DocumentPredicate>
...
std::vector<Document> SearchServer::FindAllDocuments(const std::execution::parallel_policy&, const Query& query, DocumentPredicate document_predicate) const {
    ...
    auto result_plus_word = [this, &document_predicate, &document_to_relevance](std::string_view word) {
        if (word_to_document_freqs_.count(word) == 0) {
            return;
        }
    ...
    for_each(std::execution::par, query.plus_words.begin(), query.plus_words.end(), result_plus_word);

    auto result_minus_word = [&](std::string_view word) {
        if (word_to_document_freqs_.count(word) == 0) {
            return;
        }
    ...
    for_each(std::execution::par, query.minus_words.begin(), query.minus_words.end(), result_minus_word);

    const auto& document_build_map = document_to_relevance.BuildOrdinaryMap();
    std::vector<Document> matched_documents;

    for (const auto& [document_id, relevance] : document_build_map) {
